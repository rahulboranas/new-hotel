...
function Home() {
  return (
    <div className="p-4 select-none container mx-auto">
      ...
      <div className="grid grid-cols-4 gap-2">
        {categories.map((cat) => (
          ...
          <Link key={cat.id} to={`/category/${cat.id}`} className="rounded-full w-full flex flex-col items-center py-2 shadow-sm hover:shadow-md transition-all bg-white dark:bg-gray-800 dark:border dark:border-gray-700">
            <img src={cat.img} alt={cat.name} className="w-15 h-15 object-cover rounded-full" />
